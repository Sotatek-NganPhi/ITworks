...
trait Criteriable
{
    public function updateRelations($model, $attributes)
    {
            ...
            if (isset($model) &&
                ...
                switch ($methodClass) {
                    case 'Illuminate\Database\Eloquent\Relations\BelongsToMany':
                        ...
                        $pivotChanges[] = $model->$key()->sync(array_values($new_values));
                        break;
        ...
        $changedRecords = collect($pivotChanges)->reduce(function ($carry, $change) {
            return $carry + count($change['attached']) + count($change['detached']) + count($change['updated']);
        });

        if ($changedRecords > 0) {
            event(new CriteriaChangedEvent($model));
        }
