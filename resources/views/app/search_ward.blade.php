...
@section('vue-js')
    <script src="/js/app/home/search_ward.js"></script>
@endsection
...
@section('page_content')
    ...
    <div id="search">
        ...
        <div class="searchPanel">
            <ul class="tabArea">
                <li class="tab {{preg_match('/wards/', Request::fullUrl()) ? 'selected' : ''}}"><a
            ...
            <div class="contents contentsArea">
                <div class="title">ご希望の市区町村名を選択してください</div>
                <div class="inputArea">
                    <ul class="inputBase">
                        @if(isset($wards))
                            @foreach($wards as $w)
                                <li class="input">
                                    <label for="{{$w->id}}" title="{{$w->name}}">
                                        <input type="checkbox" value="{{$w->id}}" name="{{$w->name}}"
                                               id="{{$w->id}}" v-model="wardSelected"
                                                {{is_array($wardSelected) ?
                                                (in_array($w->name_en, $wardSelected) ? 'checked' : '') :
                                                ($wardSelected == $w->name_en ? 'checked' : '') }}>
                                        {{str_limit($w->name, 13, '..')}}（<span class="num">{{$w->total_job}}</span>)
                                    </label>
                                </li>
                            @endforeach
                        @endif
        ...
        @endif
        @if(isset($wards))
            <input hidden value="{{ $wards }}" name="allWard">
        @endif
        @include('app.common.breadcrumbs', ['crumbs' => collect([
            ['url' => route("home.index"), 'name' => $configs["site_name"]],
            ['url' => route("home", $region->key), 'name' => $region->name],
